...
public:
    ...
    std::optional<EEGStatePrediction> predict(const std::vector<std::vector<float>>& eegChannels,
                                              float sampleRateHz) override;

    bool isRealtimeCapable() const override
    {
        return true;
    }

    void setSimulate(bool v)
    {
        simulate_ = v;
    }
    void setFixedTargetHz(float hz)
    {
        fixedTargetHz_ = hz;
    }
