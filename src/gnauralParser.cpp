...
bool parseTxtFormat(std::istream& in, Program& out)
{
    ...
    while (std::getline(in, line)) {
        ...
        float freqL, freqR;
        int dur;
        if (std::sscanf(line.c_str(), "%f, %f, %d", &freqL, &freqR, &dur) == 3 && dur > 0) {
...
bool parseXmlFormat(const std::string& content, Program& out)
{
    ...
    std::regex entryRe(
        "<entry[^>]*duration=\"([0-9.]+)\"[^>]*beatfreq=\"([0-9.]+)\"[^>]*basefreq=\"([0-9.]+)\"[^>"
        "]*volume_left=\"([0-9.]+)\"[^>]*volume_right=\"([0-9.]+)\"[^>]*state=\"([0-9]+)\"");
    std::regex entryRe2(
        "<entry[^>]*beatfreq=\"([0-9.]+)\"[^>]*basefreq=\"([0-9.]+)\"[^>]*duration=\"([0-9.]+)\"[^>"
        "]*");
    std::regex entryRe3(
        "<entry[^>]*parent=\"[^\"]*\"[^>]*duration=\"([0-9.]+)\"[^>]*volume_left=\"([0-9.]+)\"[^>]*"
        "volume_right=\"([0-9.]+)\"[^>]*beatfreq=\"([0-9.]+)\"[^>]*basefreq=\"([0-9.]+)\"[^>]*"
        "state=\"([0-9]+)\"");
    ...
    while (std::regex_search(s, m, entryRe)) {
        addEntry(safeStof(m[1].str()), safeStof(m[2].str()), safeStof(m[3].str()),
                 safeStof(m[4].str()), safeStof(m[5].str()), safeStoi(m[6].str(), 1));
        s = m.suffix();
    }
    if (beatFreqs.empty()) {
        s = content;
        while (std::regex_search(s, m, entryRe3)) {
            addEntry(safeStof(m[1].str()), safeStof(m[4].str()), safeStof(m[5].str()),
                     safeStof(m[2].str()), safeStof(m[3].str()), safeStoi(m[6].str(), 1));
            s = m.suffix();
        }
    }
    if (beatFreqs.empty()) {
        ...
        while (std::regex_search(s, m, entryRe2)) {
            addEntry(safeStof(m[3].str()), safeStof(m[1].str()), safeStof(m[2].str()), 0.85f, 0.85f,
                     1);
            s = m.suffix();
        }
    }
    ...
    for (size_t i = 0; i < beatFreqs.size(); ++i) {
        ...
        if (isPinkNoiseOnly) {
            ...
            p.background =
                (noiseVol > 0.001f) ? Period::Background::PinkNoise : Period::Background::None;
            p.backgroundVol = noiseVol;
        }
        out.seq.push_back(std::move(p));
    }
