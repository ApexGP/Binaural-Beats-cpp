...
{
    std::cout << "Binaural beats CLI. Plays or writes WAV with configurable "
                 "parameters.\n"
              << "Usage: " << prog << " [options]\n\n"
              << "Options:\n"
...
static bool parseInt(const char *s, int &out)
{
    ...
    if (end == s || *end != '\0' || v < 0 || v > 86400) return false;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (std::strcmp(arg, "--beatFreq") == 0) {
            ...
            if (!parseFloat(argv[++i], beatFreq) || beatFreq < 0.f || beatFreq > 40.f) {
        ...
        if (std::strcmp(arg, "--baseFreq") == 0) {
            ...
            if (!parseFloat(argv[++i], baseFreq) || baseFreq < 0.f || baseFreq > 500.f) {
        ...
        if (std::strcmp(arg, "--noiseVol") == 0) {
            ...
            if (!parseFloat(argv[++i], noiseVol) || noiseVol < 0.f || noiseVol > 1.f) {
    ...
    if (auto *wav = dynamic_cast<WavFileDriver *>(driver.get())) {
        auto wavCb = [&synth, &config](std::vector<int16_t> &buf) {
            ...
            synth.advanceTime(static_cast<float>(config.bufferFrames) / config.sampleRate);
        };
        wav->start(config.sampleRate, config.bufferFrames, wavCb);
        int totalSec = 0;
        for (const auto &p : program.seq) totalSec += p.lengthSec;
        if (totalSec <= 0) totalSec = durationSec;
        std::cout << "PortAudio not found. Writing output.wav (" << totalSec << " sec)...\n";
    ...
    auto startPlayback = [&]() {
        return driver->start(
            config.sampleRate, config.bufferFrames, [&synth, &config](std::vector<int16_t> &buf) {
                synth.skewVoices(synth.periodElapsedSec());
                synth.fillSamples(buf);
                synth.advanceTime(static_cast<float>(config.bufferFrames) / config.sampleRate);
            });
    ...
    if (loadedFromGnaural)
        std::cout << "Playing in " << program.name << ". Press Enter to pause, Q to quit.\n";
    else
        std::cout << "Playing: " << beatFreq << " Hz beat, " << baseFreq << " Hz base ("
                  << durationSec << " s). Press Enter to pause, Q to quit.\n";
    bool playing = true;
    bool quit = false;
    ...
    while (!quit) {
        ...
        if (select(STDIN_FILENO + 1, &fds, nullptr, nullptr, &tv) > 0) {
            int c = getchar();
            if (c != EOF) key = static_cast<unsigned char>(c);
        }
#endif
        if (key == 'q' || key == 'Q') {
            quit = true;
        ...
        if (key == '\n' || key == '\r') {
            if (playing) {
                if (!loadedFromGnaural)
                    totalElapsed +=
                        std::chrono::duration<float>(std::chrono::steady_clock::now() - playStart)
                            .count();
                driver->stop();
                playing = false;
            } else {
                if (!loadedFromGnaural && totalElapsed >= static_cast<float>(durationSec)) {
                    continue;
                }
                if (startPlayback()) {
                    playing = true;
                    if (!loadedFromGnaural) playStart = std::chrono::steady_clock::now();
                } else {
        ...
        if (loadedFromGnaural && !program.seq.empty()) {
            ...
            elapsed = playing ? totalElapsed + std::chrono::duration<float>(
        ...
        int stateCode =
            playing
                ? 0
                : (!loadedFromGnaural && totalElapsed >= static_cast<float>(durationSec) ? 2 : 1);
        if (displaySec != lastDisplayedSec || displayTotal != lastDisplayedTotal ||
            ...
            const char *state = (stateCode == 0)   ? "[Playing]"
                                : (stateCode == 2) ? "[Done]   "
                                                   : "[Paused] ";
            std::cout << "\rduration " << displaySec << " / " << displayTotal << " s  " << state
                      << "  " << std::flush;
        }
