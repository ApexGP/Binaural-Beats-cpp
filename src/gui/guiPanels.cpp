...
void renderTitleBar(AppContext &ctx)
{
    ...
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("Help Center");
    ...
    ImGui::SameLine();
    ImGui::TextDisabled((ctx.loadedFromGnaural || ctx.timedPlaybackEnabled) ? " / %s s" : " / %s",
                        tBuf);
    ImGui::EndGroup();
    ...
    if (ImGui::BeginPopup("MenuPopup")) {
        if (ImGuiWindow *w = ImGui::GetCurrentWindow()) w->WindowRounding = 12.f;
...
void renderWaveform(AppContext &ctx)
{
    ...
    if (!samplesL.empty()) {
        ...
        ImGui::PlotLines("##L", samplesL.data(), static_cast<int>(samplesL.size()), 0, nullptr,
                         -1.f, 1.f, ImVec2(-1, h));
        ImGui::PlotLines("##R", samplesR.data(), static_cast<int>(samplesR.size()), 0, nullptr,
                         -1.f, 1.f, ImVec2(-1, h));
        ImGui::PopStyleColor();
    } else {
...
void renderBeatDescription(AppContext &ctx)
{
    ...
    ImGui::PushTextWrapPos(ImGui::GetContentRegionAvail().x + ImGui::GetCursorScreenPos().x);
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.9f, 1.f), "%s", getBeatDescription(ctx.beatFreq));
...
void renderControls(AppContext &ctx)
{
    ...
    if (sliderWithButtons("Binaural Beat", &ctx.beatFreq, BEAT_MIN, BEAT_MAX, "%.3f Hz", 0.5f, buf,
                          "%.3f", "Hz", s)) {
        if (!ctx.loadedFromGnaural) ctx.manualElapsedSec.store(0.f, std::memory_order_relaxed);
    ...
    if (sliderWithButtons("Base Frequency", &ctx.baseFreq, BASE_FREQ_MIN, BASE_FREQ_MAX, "%.3f Hz",
                          5.f, buf, "%.3f", "Hz", s)) {
        if (!ctx.loadedFromGnaural) ctx.manualElapsedSec.store(0.f, std::memory_order_relaxed);
        if (!ctx.program.seq.empty() && curIdx < static_cast<int>(ctx.program.seq.size()) &&
            !ctx.program.seq[curIdx].voices.empty()) {
    ...
    if (sliderWithButtons("Balance", &ctx.balance, BALANCE_MIN, BALANCE_MAX, "%.3f", 0.1f,
                          getBalanceLabel(ctx.balance), "%.3f", "", s)) {
        if (!ctx.loadedFromGnaural) ctx.manualElapsedSec.store(0.f, std::memory_order_relaxed);
    ...
    if (!ctx.program.seq.empty() && curIdx < static_cast<int>(ctx.program.seq.size()) &&
        ...
        if (ImGui::Combo("Background", &bg, bgNames, 3)) {
            if (!ctx.loadedFromGnaural) ctx.manualElapsedSec.store(0.f, std::memory_order_relaxed);
            ctx.program.seq[curIdx].background = static_cast<binaural::Period::Background>(bg);
            ctx.synth.setProgram(ctx.program);
        }
        if (ctx.program.seq[curIdx].background != binaural::Period::Background::None) {
            ImGui::Spacing();
            ImGui::AlignTextToFramePadding();
            ImGui::Text("Noise");
            ImGui::SameLine(70 * s);
            float noiseW = ImGui::GetContentRegionAvail().x - 20.f * s;
            if (noiseW < 80.f * s) noiseW = 80.f * s;
            ImGui::PushID("NoiseVol");
            ImGui::SetNextItemWidth(noiseW);
            if (ImGui::SliderFloat("##noise", &ctx.program.seq[curIdx].backgroundVol, 0.f, 1.f,
                                   "%.2f")) {
...
void renderHelpCenter(AppContext &ctx)
{
    ...
    bool visible = ImGui::Begin("Help Center", &ctx.showHelpCenter, ImGuiWindowFlags_NoCollapse);
    if (visible) {
        if (ImGui::BeginChild("HelpContent", ImVec2(0, -30 * s), ImGuiChildFlags_Border)) {
            ...
            ImGui::TextWrapped(
                ...
                "from 40 to 500 Hz. ");
            ImGui::TextWrapped("Common: 161 Hz or 200 Hz. Too high may cause discomfort.");
            ImGui::Spacing();
            ImGui::TextWrapped(
            ...
            ImGui::TextWrapped(
                "Isochronic: Adds pulsed beats when enabled, "
                "works without headphones.");
            ImGui::TextWrapped(
                "Background: Pink noise, white noise, etc., "
                "useful for meditation.");
            ImGui::Spacing();
            ImGui::TextColored(ImVec4(0.4f, 0.7f, 1.f, 1.f), "Example Presets");
            ImGui::Separator();
            ImGui::TextWrapped("Meditation/Relax: 4 Hz(Theta), base 161 Hz, 20 to 40 minutes.");
            ImGui::TextWrapped("Focus/Study: 12 Hz(Alpha) or 14 Hz(Beta), base 200 Hz.");
            ImGui::TextWrapped("Deep sleep: 2 Hz(Delta), low volume, with pink noise.");
            ImGui::TextWrapped("Creativity/Light sleep: 6 Hz(Theta), enable isochronic.");
            ImGui::Spacing();
        ...
        if (ImGui::Button("Close", ImVec2(80 * s, 0))) ctx.showHelpCenter = false;
    }
    ImGui::End();
    ImGui::PopStyleVar();
}
...
void renderLoadModal(AppContext &ctx)
{
    ...
    if (ImGui::BeginPopupModal("Load Gnaural", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ctx.modalOpen = true;
        ...
        if (ImGui::Button("Browse...")) {
            ...
            ofn.lpstrFilter = L"Gnaural (*.txt;*.gnaural)\0*.txt;*.gnaural\0All (*.*)\0*.*\0";
            ofn.lpstrFile = pathW;
            ofn.nMaxFile = 512;
            ...
            if (GetOpenFileNameW(&ofn)) {
                ...
                int needed =
                    WideCharToMultiByte(CP_UTF8, 0, pathW, -1, nullptr, 0, nullptr, nullptr);
                if (needed > 0) {
                    std::vector<char> pathA(static_cast<size_t>(needed));
                    WideCharToMultiByte(CP_UTF8, 0, pathW, -1, pathA.data(), needed, nullptr,
                                        nullptr);
...
void renderTimedPlaybackModal(AppContext &ctx)
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_PopupRounding, 12.f);
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 12.f);
    if (ImGui::BeginPopupModal("Timed Playback", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ctx.modalOpen = true;
        ImGui::Text("Set playback duration (seconds):");
        ImGui::SetNextItemWidth(120 * s);
        ImGui::InputFloat("##duration", &ctx.timedPlaybackDurationSec, 60.f, 300.f, "%.0f",
                          ImGuiInputTextFlags_CharsDecimal);
        if (ctx.timedPlaybackDurationSec < 1.f) ctx.timedPlaybackDurationSec = 1.f;
        ImGui::Spacing();
