...
const char *getBeatDescription(float hz)
{
    ...
    if (hz < 8.f) return "Theta waves are linked to meditation, creativity, and light sleep.";
    if (hz < 12.f)
...
{
    if (b < -0.3f) return "Left";
    if (b > 0.3f) return "Right";
    return "Center";
}
...
bool sliderWithButtons(const char *label, float *v, float minV, float maxV, const char *fmt,
                       float step, const char *valueLabel, const char *inputFmt,
                       const char *unitSuffix, float scale)
{
    bool changed = false;
    ...
    if (ImGui::SliderFloat("##slider", v, minV, maxV, nullptr, ImGuiSliderFlags_NoInput)) {
        changed = true;
    }
    ImGui::SameLine();
    if (ImGui::Button("+", ImVec2(btnSz, btnSz))) {
