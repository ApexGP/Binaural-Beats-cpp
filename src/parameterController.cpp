...
ParameterController::ParameterController(Synthesizer &synth, PredictionQueue &queue)
...
void ParameterController::update(float periodElapsedSec)
{
    ...
    if (lastPrediction_ && lastPrediction_->confidence > 0.01f) {
        ...
        float target = std::clamp(lastPrediction_->targetBeatFreq, BEAT_FREQ_MIN, BEAT_FREQ_MAX);
